...
# BULK ADD
@api_view(['POST'])
@permission_classes([IsAdminUser])  # Only admins should be able to upload
def bulk_add_questions(request):
    ...
    questions_created = 0  # Track how many questions were successfully created
    error_rows = []  # Track rows that failed to save

    for index, row in enumerate(csv_reader, start=1):
        try:
            # Create a new Question object for each row in the CSV with default values for certain fields
            question = Question(
                ...
                difficulty=row.get('difficulty', 'easy'),  # Default to 'easy' if not provided
                net_rating=0.0,  # Default value as specified
                rated_count=0  # Default value as specified
            )
            question.save()
            questions_created += 1
        except Exception as e:
            # Append any error rows with their row index for easy debugging
